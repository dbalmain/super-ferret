...
//! `cargo run -p ferret-bench --release --bin gitignore_vs_ignore`.

// The benchmark compiles ferret-policy's private matcher in place so production
// does not expose a benchmark-only API. Re-inclusion-only methods are unused here.
#[allow(dead_code)]
#[path = "../../../ferret-policy/src/gitignore/mod.rs"]
mod gitignore;
